...
class Project final {
 ...
 private:
  ...
  const std::string kCMake =
      "project(@)\n\n"
      "if (DIPC_CDT_ROOT STREQUAL \"\" OR NOT DIPC_CDT_ROOT)\n"
      //"  find_package(dipc.cdt)\n"
      "  find_package(dipc.cdt)\n"
      "else()\n"
      "  list(APPEND CMAKE_MODULE_PATH "
      "${DIPC_CDT_ROOT}/lib/cmake/dipc.cdt)\n"
      "  include(DipcWasmToolchain)\n"
      "  include(DipcCDTMacros)\n"
      "endif()\n\n"
...
//      "  find_package(dipc.cdt)\n"
      ...
      "  CMAKE_ARGS "
      "-DCMAKE_TOOLCHAIN_FILE=${DIPC_CDT_ROOT}/lib/cmake/dipc.cdt/"
      "DipcWasmToolchain.cmake -DDIPC_CDT_ROOT=${DIPC_CDT_ROOT}\n"
      "  UPDATE_COMMAND \"\"\n"
      "  PATCH_COMMAND \"\"\n"
      "  TEST_COMMAND \"\"\n"
      "  INSTALL_COMMAND \"\"\n"
  ...
  const std::string kReadmeCMake =
      ...
      "directory\n"
      "   - You can then deploy contract\n\n"
      " - Additions to CMake should be done to CMakeLists.txt in the './src' "
      "directory and not in the top level CMakeLists.txt";

  const std::string kReadme =
      "--- @ Project ---\n\n"
      "- How to Build -\n"
      "  - run the command 'dipc-cpp -o @.wasm @.cpp -abigen'\n";
};

int main(int argc, const char** argv) {
  llvm::cl::SetVersionPrinter([](llvm::raw_ostream& os) {
    os << "dipc-init version "
       << "@VERSION_FULL@" << '\n';
  });
  llvm::cl::OptionCategory cat("dipc-init",
                               "generates an dipc smart contract project");
  ...
  llvm::cl::ParseCommandLineOptions(argc, argv, std::string("dipc-proj"));
  try {
