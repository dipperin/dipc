...
#define DIPC_REFLECT_MEMBER_OP( r, OP, elem ) \
  OP t.elem
 ...
 */
#define DIPC_SERIALIZE( TYPE,  MEMBERS ) \
 template<typename DS> \
 friend DS& operator << ( DS& ds, const TYPE& t ){ \
    return ds BOOST_PP_SEQ_FOR_EACH( DIPC_REFLECT_MEMBER_OP, <<, MEMBERS );\
 }\
 template<typename DS> \
 ...
 *  @param MEMBERS - a sequence of member names.  (field1)(field2)(field3)
 */
#define DIPC_SERIALIZE_DERIVED( TYPE, BASE, MEMBERS ) \
 template<typename DS> \
 friend DS& operator << ( DS& ds, const TYPE& t ){ \
