...
//            }
            ...
            const Key& first() const {
                DipcAssert(first_ != nullptr, "first is nullptr");
                return *first_;
            }
            ...
            const Value& second() const {
                DipcAssert(second_ != nullptr, "second is nullptr");
                return *second_;
            }
        private:
            const Key *first_;
        ...
        public:
            ...
            /**
             * @brief Construct a new dipc serialize object
             * 
             * @param name_ 
             */
            DIPC_SERIALIZE(KeyWrapper, (name_)(key_))
        private:
...
//            }
    ...
    public:
        ...
        Value getConst(const Key &k) {
            ...
            Value v;
            dipc::getState(KeyWrapper(keySetName_, k), v);
            return v;
        }
...
//        }
        ...
        size_t size() {
            init();
            DipcAssert(type == MapType::Traverse, "NoTraverse of Map", keySetName_);
            return keySet_.size();
        }
        /**
        ...
        void flush() {
            std::for_each(
                    ...
                    [this](const Key &k) {
                        auto iter = map_.find(k);
                        if (iter != map_.end()) {
                            dipc::setState(KeyWrapper(keySetName_, k), iter->second);
                        } else {
                            dipc::delState(k);
                            if (type == MapType::Traverse) {
                                keySet_.erase(k);
    ...
    private:
        ...
        void init() {
            if (!init_ && type == MapType::Traverse) {
                dipc::getState(keySetName_, keySet_);
                init_ = true;
            }
        }
