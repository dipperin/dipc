...
namespace dipc {
    ...
    public:
        ...
        public:
            ...
            Iterator operator --(int) {
                DipcAssert(pos_ > 0, "pos can't be negative");
    ...
    public:
        ...
        Key& get(size_t index) {
            DipcAssert(index < size_, "out of range", "index:", index, "size:", size_);
            ...
            if (getState(key, res) == 0) {
                dipcThrow("getState error list name:", name_, "index:", index, "mark pos;", i);
            }
        ...
        void flush() {
            for (auto it : cache_) {
                if (it.second.getState() == DEL) {
                    dipc::delState(encodeKey(it.first));
