...
EXPORT void TestToken::init(std::string tokenName, const std::string sym, uint64_t supply){
...
EXPORT void TestToken::approve(const std::string spender, uint64_t value){
    Address callerAddr = caller();
    std::string spenderStr = StrToAddress2Str(spender);
...
PAYABLE void TestToken::transfer(const std::string to, uint64_t value){
    Address callerAddr = caller();
    ...
    std::string toStr = StrToAddress2Str(to);
...
WITHDRAW void TestToken::transferFrom(const std::string from, const std::string to, uint64_t value){
    Address callerAddr = caller();
    std::string fromStr = StrToAddress2Str(from);
    std::string toStr = StrToAddress2Str(to);
...
PAYABLE uint64_t TestToken::getBalance(const std::string own){
    
    //prints(own);
    std::string ownerStr = StrToAddress2Str(own);
...
CONSTANT uint64_t TestToken::getApproveBalance(const std::string from, const std::string approved){
    //prints("getApproveBalance");
    //prints(from);
   // prints(approved);
    std::string fromStr = StrToAddress2Str(from);
    std::string approvedStr = StrToAddress2Str(approved);
