...
void convert::getBlockInfo()
{
    ...
    std::string coinbase = dipc::coinbase().toString();
    Address addr = dipc::address();
    std::string address = addr.toString();
    std::string balance = dipc::balance(addr).convert_to<std::string>().c_str();
    std::string origin = dipc::origin().toString();
    std::string caller = dipc::caller().toString();
...
void convert::rlpTest()
{
    ...
    RLPStream stream;
    stream << Address("0x00005586B883Ec6dd4f8c26063E18eb4Bd228e59c3E9", true);
