...
inline void approveIn(uint64_t change, uint64_t value){
     Address callerAddr = caller();
}
...
EXPORT void TestToken::approve(const char* spender, uint64_t value){
    ...
    approveIn(total, total);
    DIPC_EMIT_EVENT(Approval, &(callerAddr.toString()[0]), spender, value);
}

// void TestToken::approve(uint64_t change, uint64_t value){
//     Address callerAddr = caller();
// }
...
WITHDRAW void TestToken::transferFrom(const char* from, const char* to, uint64_t value){
    Address callerAddr = caller();
